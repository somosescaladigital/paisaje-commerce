  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
      {products.map((product) => (
        <div key={product.id} className="bg-white group rounded-3xl border border-light-green/20 overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col h-full">
          ...
          <div className="p-6 flex flex-col flex-1">
            ...
            {product.descripcion && (
              <p className="font-sans text-sm text-primary/60 line-clamp-3 mb-6 flex-1 italic">
                {product.descripcion}
              </p>
            )}
